...
**Форма події ігнорувала довідники (виправлено).** `EventForm` мав зашитий
`<select>` на два типи (`A`/`B`) і текстове поле для експлуатанта, хоча
Хвиля 10 дала повний CRUD довідників. Підрозділ, чиї реальні коди не «A» та
«E-01», не міг подати подію через UI взагалі: сервер відхиляв код — а до
виправлення черги подія після цього ще й зникала. Класифікатор, який
адміністратор веде, марний, якщо форма його не читає.

- Тип виробу, експлуатант і причина втрати/ремонту — тепер `<select>` з
  `/dictionaries/*`; біля причини показано її **зону**, бо саме вона визначає
  віднесення до обслуги.
- Порожній чи недоступний довідник **називається вголос** і блокує подання —
  раніше це виглядало б як порожній список і «сам винен».
- Значення результату українською (були `Success`/`Lost`/`Repair`).
- Додано **«Зрив до запуску»** (+ причина) і примітку: `aborted` доти можна
  було внести лише імпортом, тож чесний знаменник успішності через UI не
  формувався в принципі.
- Тип/експлуатант/дата зберігаються між поданнями — серія подій з одного
  вильоту вводиться швидше й з меншим ризиком описки.
- Тести: `EventForm.test.tsx` (5).

**Вивантаження звітів у продуктиві повертало 401 (виправлено).** XLSX/PDF
віддавались звичайним `<a href>`, а посилання не може нести заголовок
`Authorization`. З увімкненим глобальним auth-gate кожне «Завантажити»
відповідало 401, і документація радила «вивантажуйте через API з токеном» —
тобто аналітику пропонувалось користуватись curl. Це не обхід, це нереалізована
функція.

- `lib/download.ts` тягне файл через `fetch` з токеном і віддає blob браузеру.
  Токен лишається **в заголовку**: у query-рядку він потрапив би в історію
  браузера, логи проксі й `Referer`, а це повна сесія.
- 401 чистить сесію й повертає на вхід (раніше тіло помилки збереглося б на
  диск під іменем звіту), 403 і 5xx показуються словами.
- `SettingsPage` (резервна копія) — єдине місце, що вже робило це правильно, —
  переведено на той самий хелпер замість локальної копії.
- Тести: `download.test.ts` (6), зокрема перевірка, що токен не потрапляє в URL.

**Форми №440 існували, але до них не було жодної кнопки (виправлено).**
Генератори (`inventory.xlsx`, `movement.xlsx`, акти списання й ремонту `.docx`)
працювали з моменту появи `services/mod440.py`, а «Налаштування» показували їх
як готову функцію — при цьому дістатись до них можна було лише зібравши
HTTP-запит руками. Функція, яку нікому натиснути, не є поставленою.

Дорогою довелось закрити те, що це блокувало:

- **`GET /events` віддавав числові FK.** `UsageEventOut` містив `item_id` /
  `operator_id`, і `EventsPage` друкував їх буквально — «#24 · #10». У системі
  **пономерного** обліку список подій не показував жодного серійного номера, і
  обрати подію для акта було фізично неможливо. Доданий `UsageEventListOut`
  резолвить серійний номер, коди типу, експлуатанта й причини (LEFT JOIN, щоб
  успішні події без причини не випадали). Шлях запису (`UsageEventOut`) не
  чіпали — `sync.ts` читає звідти лише `id`.
- **`Mod440Page`**: чотири форми, найменування частини вводиться раз і
  запамʼятовується (localStorage), подія для акта обирається за серійним
  номером. Події **без проставленої причини не пропонуються** — генератор
  відповів би 400.
- Demo-мок `events.json` перегенеровано в новий формат (стабільний seed).
- Тести: `Mod440Page.test.tsx` (5), `test_events.py` +2.
...
- **ADR-021 (Wave 6)** — проактивні сигнали — окрема легка сутність
  ...
  а не автоматична.
- **ADR-024 (Wave 13)** — якість рішення — окреме поле на кейсі, ортогональне
  до наслідку (`decision_quality` + `decision_rationale`). *Чому:* платформа вже
  проводить цю межу на Рівні 1 — зона відповідальності відділяє «що обслуга
  контролювала» від «що з нею сталося», і саме тому існує η_c. На Рівні 2
  еквівалента не було: кейс фіксував, що сталося, і жодного разу не казав, чи
  було рішення виправданим за тим, що було відомо. Оцінка за наслідком вчить
  боятися випадковості, а не думати (NATO LLH4, TC 25-20).
  *Що НЕ робимо:* не гейтимо перехід у `endorsed` за наявністю оцінки —
  замість жорсткої заборони цикл навчання рахує «виправляють, не оцінивши».
  Заборона змусила б ставити оцінку формально; лічильник показує проблему,
  лишаючи рішення людині.
  *Похідне:* `caught_before_it_cost_anything` — кейси, відкриті **людиною**
  (тригер `manual`, тобто показники були в нормі) з хибним чи лише прийнятним
  рішенням. Це та сама «пощастило» — найцінніший і зазвичай ніде не записаний
  клас спостережень.

- **ADR-025 (Wave 13)** — знання старіє за категоріями; чинність **обчислюється**,
  ніколи не зберігається як стан. *Чому:* ADR-009 пускає в брифінг лише
  `validated`, і жоден актив не спливав — урок, валідований два роки тому,
  подавався планувальнику як чинна істина. Для патерну РЕБ це не «застарілий
  рядок», а брифінг, що стверджує неправду про противника, якого вже немає.
  Період піврозпаду задано **на тип** (`failure_pattern` 90 дн. …
  `architectural_decision` 1825 дн.) з можливістю перекрити для одного активу
  (`review_after_days`).
  *Що НЕ робимо:* **жодної авто-деприкації.** Машина, що тихо списує урок,
  підтверджений людиною, — це дзеркальне відображення авто-валідації, від якої
  ми відмовились в ADR-008. Застарілий актив лишається `validated` і далі
  живить брифінг, але: позначений у `meta`, ранжується нижче свіжого за рівної
  релевантності, і `freshness` іде в LLM-пакет, а системний промпт прямо
  забороняє стверджувати `stale`-урок як поточну обстановку.
  *Людська дія:* `POST /context/assets/{id}/reaffirm` перезапускає лічильник і
  пишеться в ланцюг аудиту (`CONTEXT_ASSET_REAFFIRMED`).

- **ADR-026 (Wave 13)** — передбачуваність вимірюємо як **повторюваність
  відповіді**, а не вимогою мати ≥2 варіанти. *Чому:* початкове формулювання
  Parallax — «кожна рекомендація мусить нести дві рівноцінні альтернативи» —
  дає одне з двох: вигаданий другий варіант там, де дані підтверджують один
  курс дій, або порожню галочку. Виготовлення контенту — це рівно та вада, яку
  проєкт вичищав усю дорогу (фейковий sparkline, вигадана «довіра 0.84»).
  *Що робимо натомість:* `GET /learning/response-diversity` групує за
  (експлуатант, тригер) і показує пари, де ≥80% рекомендацій припадають на одну
  й ту саму дію (від 3 кейсів — двічі це збіг, не звичка). Це **факт у даних**.
  Система повідомляє повторення і **не робить висновку**, чи стала поведінка
  підрозділу читаною для противника: це залежить від обстановки, яку знає
  читач, а не сервіс.
  *Не плутати з `recurrence_rate`:* той каже, що **виправлення не спрацювало**
  (проблема повернулась); цей — що **відповідь не змінюється**. Цілком дієва
  рекомендація може бути цілком передбачуваною.
  *Нормалізація тексту навмисно поверхнева* (регістр, пробіли, пунктуація):
  агресивна (стемінг, синоніми) склеїла б дві різні дії в одну й вигадала б
  знахідку. Схема БД не змінювалась — усе рахується з наявних даних.

- **ADR-027 (Wave 14)** — **число не подається без свого знаменника, а цикл не
  крутиться без виходу.** *Звідки:* аналіз ТЗ паралельного проєкту (система
  підтримки рішень командира на базі Farsight Vision), принципи 5 «прозорість
  невизначеності» і 10 «ліміт ітерацій». Перевірка по нашому коду виявила два
  реальні дефекти, не гіпотетичні:
  1. `OperatorRating` не ніс розміру вибірки. Рейтинг на **5 запусках** давав
     категорію «висока готовність» у тій самій колонці, що й рейтинг на 300, і
     йшов у місячний звіт командиру. Додано `sorties` /
     `cleaned_denominator` / `sample_sufficient`; нижче `MIN_SORTIES_FOR_VERDICT`
     категорія — `insufficient_data`, тобто **система відмовляється від
     вердикту, а не вгадує його**. Тонка вибірка не може очолити таблицю:
     сортування ставить її після виміряних. Поріг — задокументована політика,
     не статистичний результат.
  2. `evidence_count` рекомендації **інкрементувався й ніколи не читався**.
     Рекомендація могла нескінченно ходити DONE → регресія → IN_PROGRESS, і
     лічильник ріс у колонці, куди ніхто не дивиться. Після
     `max_regressions` (типово 3) додано `RECOMMENDATION_ESCALATED`: тричі
     провалене виправлення — це майже завжди хибний діагноз, а не недостатньо
     застосовані ліки. Статус не змінюється — ескалація це **повідомлення
     людині**, а не вигаданий рушієм присуд.
