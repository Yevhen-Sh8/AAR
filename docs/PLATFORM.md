...
**Черга офлайн-синхронізації: тиха втрата подій (знайдено й закрито).**
`flushQueue()` читав лише індекс `status = "pending"`, а `tryPost()` ставив
`failed` на **будь-який** не-2xx, крім 401. Тобто 502 від проксі під час
деплою назавжди ховав подію: оператор бачив «подано», а запису на сервері не
було й ніколи вже не з'явилось би. Другий шлях того самого — `syncing`
виставлявся **до** `await fetch`, тож закрита вкладка лишала рядок у стані,
який теж ніхто більше не читав.

- **Класифікація помилок.** 5xx / 408 / 425 / 429 і мережеві збої —
  ретраяться (`pending`); решта 4xx — це відмова сервера від конкретних
  даних, і безкінечно слати ті самі байти безглуздо (`failed`).
- **`failed` — єдиний термінальний стан**, і потрапити в нього можна лише
  коли сервер явно відмовив. Усе, що може вдатися пізніше, лишається в черзі
  **назавжди**: для журналу втрат техніки черга, що не здається, краща за ту,
  що тихо забуває.
- **Backoff** 5 с → 10 с → 20 с … до 5 хв (`next_attempt_at`), плюс
  **таймер щохвилини** на додачу до події `online`: серверний збій події
  `online` не породжує, тож без таймера черга не рухалась.
- **Відновлення покинутих спроб:** `syncing` старший за 2 хв вважається
  загубленим і повертається в чергу. Повтор безпечний — сервер дедуплікує за
  `client_event_id`.
- **Міграція IndexedDB v1 → v2** не просто додає поля: вона **повертає в чергу
  рядки, що вже лежать у браузерах** у станах `failed` / `syncing`. Інакше
  виправлення не допомогло б тим, хто постраждав від бага.
- **UI.** Стан «Відхилено» тепер узагалі видно (раніше не рахувався ніде),
  з поясненням словами сервера і кнопками «Повторити» / «Повторити всі».
  Статуси українською.
- Тести: `sync.test.ts` (17) + `db-migration.test.ts` (1) — з перевіркою, що
  тест міграції справді падає без міграції.
