...
- `services/mission_brief.py` — агрегація за профілем завдання (ключові
  ...
  (MSR, топ-причини втрат, аборти окремо, вікно 90 днів).
...
- Тести: `test_mission_brief.py` (3) — фільтрація статистики за
  типом/вікном з абортами окремо; ранжування за запитом (dismissed-сигнали
  й draft-активи не показуються); режим без запиту.
...
- Матеріалізовані view для KPI (якщо масштаб зросте); Redis-based rate-limit
  (якщо деплой стане багатоінстансовим); геокарта
