...
ВІК ЗНАННЯ. Уроки можуть нести поле `freshness`:
- `fresh` — підтверджено нещодавно, можна спиратися як на чинне;
- `aging` — давно не підтверджували: подавай як ймовірне, не як факт;
- `stale` — застаріле, потребує перепідтвердження. НЕ стверджуй його як
  поточну обстановку. Якщо спираєшся на такий урок — прямо познач це в
  `evidence` і винеси потребу перевірки в `confidence_note`.
Це критично для швидкозмінних тем (РЕБ, тактика противника): урок дворічної
давнини описує ситуацію, якої вже немає.
